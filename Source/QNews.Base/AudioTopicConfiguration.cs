...
{
    // AudioTopic
    internal partial class AudioTopicConfiguration : EntityTypeConfiguration<AudioTopic>
    {
        public AudioTopicConfiguration(string schema = "dbo")
        {
            ToTable(schema + ".AudioTopic");
            HasKey(x => x.ID);
            ...
            // Foreign keys
            HasRequired(a => a.AspNetUser_CreateBy).WithMany(b => b.AudioTopics_CreateBy).HasForeignKey(c => c.CreateBy); // FK_AudioTopic_AspNetUsers
            HasRequired(a => a.AspNetUser_ModifyBy).WithMany(b => b.AudioTopics_ModifyBy).HasForeignKey(c => c.ModifyBy); // FK_AudioTopic_AspNetUsers1
            InitializePartial();
        }

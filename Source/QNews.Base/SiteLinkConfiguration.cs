...
{
    // SiteLink
    internal partial class SiteLinkConfiguration : EntityTypeConfiguration<SiteLink>
    {
        public SiteLinkConfiguration(string schema = "dbo")
        {
            ToTable(schema + ".SiteLink");
            HasKey(x => x.ID);
            ...
            // Foreign keys
            HasRequired(a => a.AspNetUser_CreateBy).WithMany(b => b.SiteLinks_CreateBy).HasForeignKey(c => c.CreateBy); // FK_SiteLink_AspNetUsers
            HasRequired(a => a.AspNetUser_ModifyBy).WithMany(b => b.SiteLinks_ModifyBy).HasForeignKey(c => c.ModifyBy); // FK_SiteLink_AspNetUsers1
            HasRequired(a => a.Status).WithMany(b => b.SiteLinks).HasForeignKey(c => c.StatusID); // FK_SiteLink_Status
            InitializePartial();
        }

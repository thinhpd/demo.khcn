...
{
    // DocumentIssue
    internal partial class DocumentIssueConfiguration : EntityTypeConfiguration<DocumentIssue>
    {
        public DocumentIssueConfiguration(string schema = "dbo")
        {
            ToTable(schema + ".DocumentIssue");
            HasKey(x => x.ID);
            ...
            // Foreign keys
            HasRequired(a => a.AspNetUser_CreateBy).WithMany(b => b.DocumentIssues_CreateBy).HasForeignKey(c => c.CreateBy); // FK_DocumentIssue_AspNetUsers
            HasRequired(a => a.AspNetUser_ModifyBy).WithMany(b => b.DocumentIssues_ModifyBy).HasForeignKey(c => c.ModifyBy); // FK_DocumentIssue_AspNetUsers1
            InitializePartial();
        }

...
{
    // Video
    internal partial class VideoConfiguration : EntityTypeConfiguration<Video>
    {
        public VideoConfiguration(string schema = "dbo")
        {
            ToTable(schema + ".Video");
            HasKey(x => x.ID);
            ...
            // Foreign keys
            HasRequired(a => a.AspNetUser_CreateBy).WithMany(b => b.Videos_CreateBy).HasForeignKey(c => c.CreateBy); // FK_Video_AspNetUsers
            HasRequired(a => a.AspNetUser_ModifyBy).WithMany(b => b.Videos_ModifyBy).HasForeignKey(c => c.ModifyBy); // FK_Video_AspNetUsers1
            HasRequired(a => a.Status).WithMany(b => b.Videos).HasForeignKey(c => c.StatusID); // FK_Video_Status
            HasOptional(a => a.VideoTopic).WithMany(b => b.Videos).HasForeignKey(c => c.TopicID); // FK_Video_VideoTopic
            InitializePartial();
        }
